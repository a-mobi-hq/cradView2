...
function QuestionBus() {
    ...
    const fetchUnansweredQuestion = async () => {
        try {
          const response = await fetch(API_BASE_URL+`/api/new/question/${userId}/${projectId}/ValuePropositionPack/PriceAndPricingStrategy`);
          if (response.status === 200) {
            const data = await response.json();
            if (!data.data) {
            //   setNoMoreQuestions(true);
                navigate(`/questionVppPsSum`);
            } else {
    ...
    <div className='container2'>
         ...
         <div className='text-center'>
                    <p className='textHp'>Price And Pricing Strategy,</p>
