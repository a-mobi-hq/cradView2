...
function QuestionBusIntro() {
 ...
 const questionSubType ="MarketAnalysis";
 ...
 useEffect(() => {
   const fetchAnswers = async () => {
     ...
     if(summaryResponse.ok) {
       ...
       const response = await fetch(API_BASE_URL + `/api/new/question/BusinessCaseBuilder/MarketAnalysis/${projectId}`);
       if (!response.ok) {
   ...
   if (isResizing) {
    ...
    <div className='container2'>
         ...
         <div className='text-center'>
                    <p className='textHp'>Market Analysis</p>
