...
function QuestionBusIntro() {
 ...
 const questionSubType ="RiskAndMitigationStrategies";
 ...
 useEffect(() => {
   const fetchAnswers = async () => {
     ...
     if(summaryResponse.ok) {
       ...
       const response = await fetch(API_BASE_URL + `/api/new/question/BusinessCaseBuilder/RiskAndMitigationStrategies/${projectId}`);
       if (!response.ok) {
 ...
 const onClickNext = () => navigate(`/questionBusIm`);
    ...
    <div className='container2'>
         ...
         <div className='text-center'>
                    <p className='textHp'>Risk And Mitigation Strategies</p>
