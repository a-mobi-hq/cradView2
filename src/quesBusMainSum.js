...
//    checkSpelling(newText);
 ...
 const createOrUpdateSummary = async (data) => {
   try {
     ...
     const response = await fetch(API_BASE_URL +'/api/summary', {
       ...
       body: JSON.stringify({ projectId, category, subCategory, summary,userId }),
     });
 ...
 const fetchAnswerCut = async () => {
   ...
   try {
     const response = await fetch(`${API_BASE_URL}/api/answer/${projectId}/BusinessCaseBuilder/Introduction`, {
       headers: {
         'Content-Type': 'application/json',
