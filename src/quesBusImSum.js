...
function QuestionBusIntro() {
 ...
 const questionSubType ="ImplementationPlan";
 ...
 useEffect(() => {
   const fetchAnswers = async () => {
     ...
     if(summaryResponse.ok) {
       ...
       const response = await fetch(API_BASE_URL + `/api/new/question/BusinessCaseBuilder/ImplementationPlan/${projectId}`);
       if (!response.ok) {
   ...
   if (isResizing) {
    ...
    <div className='container2'>
         ...
         <div className='text-center'>
                    <p className='textHp'>Implementation Plan</p>
