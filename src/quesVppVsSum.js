...
function QuestionBusIntro() {
 ...
 const questionSubType ="ValuePropositionStatement";
 ...
 const [showScrollableDiv, setShowScrollableDiv] = useState(false);
    ...
    const onClickNext = () => navigate(`/questionVppPd`);
 ...
 useEffect(() => {
   const fetchAnswers = async () => {
     ...
     if(summaryResponse.ok) {
       ...
       const response = await fetch(API_BASE_URL + `/api/new/question/ValuePropositionPack/ValuePropositionStatement/${projectId}`);
       if (!response.ok) {
   ...
   if (isResizing) {
    ...
    <div className='container2'>
         ...
         <div className='text-center'>
                    <p className='textHp'>Value Proposition Statement</p>
