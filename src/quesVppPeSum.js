...
function QuestionBusIntro() {
 ...
 const questionSubType ="ProofPointsOfEvidence";
 ...
 const [showScrollableDiv, setShowScrollableDiv] = useState(false);
    ...
    const onClickNext = () => navigate(`/questionVppVd`);
 ...
 useEffect(() => {
   const fetchAnswers = async () => {
     ...
     if(summaryResponse.ok) {
       ...
       const response = await fetch(API_BASE_URL + `/api/new/question/ValuePropositionPack/ProofPointsOfEvidence/${projectId}`);
       if (!response.ok) {
   ...
   if (isResizing) {
    ...
    <div className='container2'>
         ...
         <div className='text-center'>
                    <p className='textHp'>Proof Points Of Evidence</p>
